...
func newBuiltinFuncStore() *functionStore {
	...
	store.Register(builtinMinEntryOld())
	store.Register(builtinMaxEntryOld())

	store.Register(builtinContainsEntryOld())
	store.Register(builtinAppendEntryOld())

	store.Register(builtinMapEntryOld())
	store.Register(builtinFilterEntryOld())
	store.Register(builtinReduceEntryOld())

	store.Register(builtinNowEntryOld())
	store.Register(builtinParseTimeEntryOld())
...
// contains
func builtinContainsEntryOld() *functionEntry {
	l := NewFunctionEntryOld("contains", builtinContainsOld)
...
func builtinContainsOld(args ...*Object) *Object {
	if res, ok := IsArgCountEqual(2, args); !ok {
		return res
	}
	main := args[0]
...
func builtinMinEntryOld() *functionEntry {
	fe := NewFunctionEntryOld("min", builtinMinOld)
...
func builtinMinOld(args ...*Object) *Object {
	if len(args) != 2 {
		return ObjectError("function min() requires a single argument. got=%d", len(args))
	}
	bothInt := (args[0].Type() == args[1].Type()) && (args[0].Type() == string(INTEGER))
...
func builtinMaxEntryOld() *functionEntry {
	fe := NewFunctionEntryOld("max", builtinMaxOld)
	fe.SetArgument("num1", INTEGER, FLOAT)
	fe.SetArgument("num2", INTEGER, FLOAT)
	fe.SetReturn("minimum", INTEGER, FLOAT)
	return fe
}
func builtinMaxOld(args ...*Object) *Object {
	if res, ok := IsArgCountEqual(2, args); !ok {
		return res
	}
	bothInt := (args[0].Type() == args[1].Type()) && (args[0].Type() == string(INTEGER))
...
// map
func builtinMapEntryOld() *functionEntry {
	fe := NewFunctionEntryOld("map", builtinMapOld)
...
func builtinMapOld(args ...*Object) *Object {
	if res, ok := IsArgCountEqual(2, args); !ok {
		return res
	}
	arrowFn, err := args[1].AsArrowFunction()
	...
	case string(ARRAY):
		...
		for idx, entry := range in {
			...
			toAdd, ok := m["return"]
			if !ok { // if no return is set, just use the existing value
				ret = append(ret, entry)
				continue
			}
			ret = append(ret, toAdd)
...
// filter
func builtinFilterEntryOld() *functionEntry {
	fe := NewFunctionEntryOld("filter", builtinFilterOld)
...
func builtinFilterOld(args ...*Object) *Object {
	if res, ok := IsArgCountEqual(2, args); !ok {
		return res
	}
	arrowFn, err := args[1].AsArrowFunction()
...
// reduce
func builtinReduceEntryOld() *functionEntry {
	fe := NewFunctionEntryOld("reduce", builtinReduceOld)
...
func builtinReduceOld(args ...*Object) *Object {
	if res, ok := IsArgCountEqual(3, args); !ok {
		return res
	}
	arrowFn, err := args[2].AsArrowFunction()
...
// append
func builtinAppendEntryOld() *functionEntry {
	fe := NewFunctionEntryOld("append", builtinAppendOld)
...
func builtinAppendOld(args ...*Object) *Object {
	if res, ok := IsArgCountEqual(2, args); !ok {
		return res
	}
	arr, err := args[0].AsArray()
...
func builtinParseTimeEntryOld() *functionEntry {
	fe := NewFunctionEntryOld("parse_time", builtinParseTimeOld)
...
func builtinParseTimeOld(args ...*Object) *Object {
	if res, ok := IsArgCountEqual(2, args); !ok {
		return res
	}
	a1 := args[0]
...
func builtinNowEntryOld() *functionEntry {
	fe := NewFunctionEntryOld("now", builtinNowOld)
	fe.SetReturn("current time", TIME)
	return fe
}

func builtinNowOld(args ...*Object) *Object {
	if res, ok := IsArgCountEqual(0, args); !ok {
		return res
	}
	return CastTime(time.Now().UTC())
